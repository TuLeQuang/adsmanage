...
function itemsChange(n) {
    ...
    for (var i = 1; i <= num; i++) {
        ...
        data_tg = data_tg + '{title:"'+charFormat(document.getElementById(itemTitleId).value)+'",domain:"'+document.getElementById(itemSponsorId).value+'",image:"' + document.getElementById(imgUrlId).value + '",link:"' + document.getElementById(linkClickId).value + '",content:"' + charFormat(document.getElementById(contentId).value) + '"},';
    }
    if (document.getElementById('itemsRequired').checked) {
        var imgUrlConfig = '{required:true,url:true}',
            contentConfig = '{max:' + document.getElementById('contentLenght').value + '}';
